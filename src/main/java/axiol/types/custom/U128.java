 ...
 * The type U 128.
 */
public final class U128 extends CustomNumber<U128> {
    /**
     * The constant SIZE.
     */
     ...
     */
    public static final U128 MIN_VALUE = new U128(0x0000_0000_0000_0000L, 0x0000_0000_0000_0000L);
    /**
     * The constant MAX_VALUE.
     */
    public static final U128 MAX_VALUE = new U128(0xFFFF_FFFF_FFFF_FFFFL, 0xFFFF_FFFF_FFFF_FFFFL);
     ...
     * The constant ONE.
     */
    public static final U128 ONE = new U128(0L, 1L);
     ...
     * @param low     the low
     */
    public U128(int high, int midHigh, int midLow, int low) {
        this.high = high;
    ...
    public U128(String text) {
        ...
        if (len > 0) {
            ...
            U128 result = U128.ZERO;
            while (i < len) {
                int digit = Character.digit(text.charAt(i++), 10);
                if (digit < 0) {
                    throw new NumberFormatException(text);
                }
                result = result.multiply(new U128(0, 10L))
                        .add(new U128(0, digit));
            }
    ...
    @Override
    public U128 setFromHexString(String hexValue) {
    ...
    @Override
    public U128 setFromDecimalString(String decimalValue) {
        U128 result = U128.ZERO;
        ...
        for (int i = 0; i < endIndex; i++) {
            ...
            result = result.multiply(new U128(10)).add(new U128(digit));
        }
    ...
    public U128 add(U128 other) {
        ...
        return new U128((int) partial3, (int) partial2, (int) partial1, (int) partial0);
    }
    ...
    @Override
    public U128 multiply(U128 multiplicand) {
        ...
        return new U128((int) hh, (int) mh, (int) ml, (int) ll);
    }

    @Override
    public U128 divide(U128 divisor) {
        if (divisor.isZero()) {
        ...
        for (int i = 0; i < SIZE; ++i) {
            ...
            if (numerator.high < 0) {
                remainder = remainder.or(U128.ONE);
            }
    ...
    @Override
    public U128 remainder(U128 divisor) {
        if (divisor.isZero()) {
    ...
    @Override
    public U128 shiftLeft(int shiftCount) {
        if (shiftCount == 0) {
            return this;
        ...
        return new U128(highPart, lowPart);
    }

    @Override
    public U128 shiftRight(int shiftCount) {
        if (shiftCount == 0) {
            return this;
        } else if (shiftCount >= SIZE || shiftCount <= -SIZE) {
            return ZERO; // All bits are gone
    ...
    @Override
    public U128 negate() {
        return new U128(~this.high, ~this.midHigh, ~this.midLow, ~this.low);
    }

    @Override
    public int compareTo(U128 n) {
        int compared = Long.compareUnsigned(this.getHigh(), n.getHigh());
        if (compared == 0) {
            compared = Long.compareUnsigned(this.getLow(), n.getLow());
        }
        return compared;
    }
    ...
    public U128 and(U128 other) {
        return new U128(this.high & other.high, this.midHigh & other.midHigh,
    ...
    public String toString(int radix) {
        ...
        U128 number = this;
        U128 base = new U128(radix);

        while (!number.isZero()) {
            U128 digit = number.remainder(base);
