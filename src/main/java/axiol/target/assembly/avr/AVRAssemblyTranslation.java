...
class AVRAssemblyTranslation implements AssemblyTranslation {
    ...
    public String getRawStackPtr(InstructionReference ref, int offset, AssemblyProgramElement proc) {
        return "[R29 - 0x%x]".formatted(offset);
    }

    @Override
    public String getStackPtr(InstructionReference ref, AssemblyProgramElement proc) {
        return "%s [R29 - 0x%x]".formatted(getPointerName(ref), proc.getStackOffset(ref));
    }
    ...
    public int getPointerSize() {
        return 8; // AVR typically uses 16-bit pointers
    }
}